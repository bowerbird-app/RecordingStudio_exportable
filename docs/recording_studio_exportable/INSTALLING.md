...
This guide explains how to install the RecordingStudioExportable engine in your Rails application.
...
```ruby
# From GitHub
gem "recording_studio_exportable", github: "bowerbird-app/recording_studio_exportable"

# Or from a local path (for development)
gem "recording_studio_exportable", path: "../recording_studio_exportable"

# Or from RubyGems (after publishing)
gem "recording_studio_exportable"
```
...
This will:
1. **Mount the engine** at `/recording_studio_exportable` in your `config/routes.rb`
2. **Create a configuration initializer** at `config/initializers/recording_studio_exportable.rb`
3. **Optionally create `config/recording_studio_exportable.yml`** for environment-specific settings
...
```ruby
mount RecordingStudioExportable::Engine, at: "/recording_studio_exportable"
```

### Configuration

Creates `config/initializers/recording_studio_exportable.rb`:
...
Create `config/initializers/recording_studio_exportable.rb`:
...
2. Visit the mounted engine root:
   ```
   http://localhost:3000/recording_studio_exportable
   ```
...
3. If the engine ships migrations, install and run them from the host app:
  ```bash
  rails generate recording_studio_exportable:migrations
...
```ruby
# Mount at root
mount RecordingStudioExportable::Engine, at: "/"

# Mount at a custom path
mount RecordingStudioExportable::Engine, at: "/my-engine"

# Mount with constraints
mount RecordingStudioExportable::Engine, at: "/recording_studio_exportable", constraints: { subdomain: "api" }
```
...
```erb
<%= link_to "Visit Engine", recording_studio_exportable.root_path %>
```

From controllers:

```ruby
redirect_to recording_studio_exportable.root_path
```

The `recording_studio_exportable` helper provides access to all engine routes.
...
| Generator fails | Check that the gem is installed: `bundle show recording_studio_exportable`. |
...
2. Delete `config/initializers/recording_studio_exportable.rb`
