...
RecordingStudioExportable provides a powerful hook system that allows host applications to customize engine behavior without modifying the engine's source code.
...
RecordingStudioExportable.configure do |config|
  config.hooks.after_initialize do
    Rails.logger.info "RecordingStudioExportable initialized!"
  end
...
```ruby
# app/hooks/recording_studio_exportable_audit_hook.rb
class RecordingStudioExportableAuditHook
...
# config/initializers/recording_studio_exportable.rb
RecordingStudioExportable.configure do |config|
  config.hooks.after_service RecordingStudioExportableAuditHook.new
end
```
...
RecordingStudioExportable.configure do |config|
  config.hooks.before_service do |service_class, args|
    Rails.logger.info "[RecordingStudioExportable] #{service_class.name} called"
  end
  
  config.hooks.after_service do |service_class, result|
    status = result.success? ? "succeeded" : "failed"
    Rails.logger.info "[RecordingStudioExportable] #{service_class.name} #{status}"
  end
end
```
...
RecordingStudioExportable.configure do |config|
  config.hooks.around_service do |service, block|
    ...
    StatsD.measure("recording_studio_exportable.#{service.class.name.demodulize.underscore}", duration)
    result
  end
end
```
...
<% content_for :recording_studio_exportable_header do %>
  <div class="custom-header">Your custom header</div>
<% end %>

<% content_for :recording_studio_exportable_footer do %>
...
```
app/views/recording_studio_exportable/home/_sidebar.html.erb  # Overrides engine partial
```
...
class HooksTest < ActiveSupport::TestCase
  setup do
    @called = false
    RecordingStudioExportable.configuration.hooks.after_initialize { @called = true }
  end
  
  test "after_initialize hook is called" do
    RecordingStudioExportable::Hooks.run(:after_initialize)
    assert @called
  end
end
```
